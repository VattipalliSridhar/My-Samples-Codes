...
public class HindiBibleBook {
    public  static HindiBibleBook book;
    ...
    public static HindiBibleBook getInstance(Context context) {
        HindiBibleBook bibleBook = book;
        return bibleBook == null ? new HindiBibleBook(context) : bibleBook;
    }

    public HindiBibleBook(Context context) {
    ...
    public void loadJSONFromAsset(Context context) {
        String str;
        try {
            InputStream open = context.getAssets().open("hindibible.json");
    ...
    public void loadOldTestanment(Context context) {
        String str;
        try {
            InputStream open = context.getAssets().open("hindioldchapters.json");
            byte[] bArr = new byte[open.available()];
            open.read(bArr);
            open.close();
            str = new String(bArr, "UTF-8");
